            ...
            <form action="/guru/sarana/pengajuan/{{ $sarana->id }}" method="post">
                ...
                {{ method_field('PATCH')}}
                 <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="nis"  value="{{$sarana->nis}}" pattern="[0-9]{3,15}" maxlength="9" required title="Nis Siswa Harus 9 Karakter Numerik" placeholder="Masukan NIS Siswa">
                    </div>
                 <div class="form-group">
                 ...
                 <div class="form-group">
                    <label for="recipient-name" class="
                    control-label">Status Pengajuan : <b style="color:red;">*</b></label></label>
                    <select name="sarana_status_pinjam_id" class="form-control" required>

                    @foreach($status as $status)
                     @if($sarana->pengajuan_status_id==$status->id)
                    <option value="{{ $status->id }}" selected="selected">{{ $status->status_pengajuan}}</option>
                    @else
                    <option value="{{ $status->id }}">{{ $status->status_pengajuan}}</option>
                    @endif
                    @endforeach
                     </select>
                  </div>
              ...
              <div class="modal-footer">
                ...
                <a href="/guru/sarana/pengajuan/{{$sarana->id}}"><button class="btn btn-success">Kembali Ke Halaman Detail Pengajuan</button> </a>
              </div>
